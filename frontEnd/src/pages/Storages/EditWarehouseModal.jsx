...
const EditWarehouseModal = ({ open, onClose, area, onSubmit }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await onSubmit(area.id, {
        ...form,
        capacity: form.capacity ? Number(form.capacity) : null
      });
      
      setErrors({});
      toast.success("Cập nhật khu vực kho thành công!");
    } catch (error) {
      toast.error("Có lỗi xảy ra khi cập nhật khu vực kho");
    } finally {
      ...
      <DialogContent sx={{ p: 0 }}>
        <Box sx={{ p: 4 }}>
          <Alert severity="info" sx={{ mb: 3, borderRadius: 2 }}>
            Cập nhật thông tin khu vực kho: <strong>{area?.name}</strong>
          </Alert>
          ...
          <Box sx={{ mt: 4, display: 'flex', gap: 2, justifyContent: 'flex-end' }}>
            ...
            >
              {isSubmitting ? "Đang cập nhật..." : "Cập nhật khu vực"}
            </Button>
          </Box>
        </Box>
      </DialogContent>
